...
window.isNebula = (window.navigator.userAgent.toString().toLowerCase().indexOf('CDII-Bucket'.toLowerCase()) != -1) || (window.navigator.userAgent.toString().toLowerCase().indexOf('nebula') != -1)
window.isNotNebula = !window.isNebula
console.log('public---agent--->：'+window.navigator.userAgent.toString())
console.log('public--->:'+(window.isNotNebula?'当前是mock环境':'当前是手机环境'))
window.csiiImageUrl = 'http://113.207.109.62:8888/public/pub/images/'
window.csiiImageCommonUrl = 'https://mcube-prod.oss-cn-hangzhou.aliyuncs.com/7D77E5C302225_default/1.0.0/commonImages/'
window.csiiUploadPath = 'http://113.207.109.62:8082/';
...
//鉴权
function AlipayJSBridge_auth(authTypes, transCode, successFun, errFun) {
  AlipayJSBridge_call('transactionCertification', {
    "authTypes": authTypes,
    "transCode": transCode
  }, function (res) {
    if (res['authState'] == 'S') {
...
function document_addEventListener(eventName, eventCallback) {
  AlipayJSBridgeReadyFun(function () {
    AlipayJSBridge.registerHandler(eventName, function (data, responseCallback) {
      ...
      eventCallback({
        data: data, preventDefault: function () {
          preventDefault = true;
          responseCallback({state: "false", preventDefault: preventDefault + ""});
        }
      });
    ...
    if (isPushWeb) {
      ...
      if (AlipayJSBridge.startupParams && AlipayJSBridge.startupParams != null) {
        ...
        if (AlipayJSBridge.startupParams.hasOwnProperty("passData") && Object.prototype.toString.call(AlipayJSBridge.startupParams["passData"]) === "[object String]") {
          alert(JSON.parse(AlipayJSBridge.startupParams["passData"]))
        ...
        if (AlipayJSBridge.startupParams.hasOwnProperty("param") && Object.prototype.toString.call(AlipayJSBridge.startupParams["param"]) === "[object String]") {
          alert("param"+JSON.parse(AlipayJSBridge.startupParams["param"]))
